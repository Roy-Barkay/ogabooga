...
var formatCardNumber = function formatCardNumber(cardNumber) {
  ...
  var maxLength = delimiter.length > 0 ? 19 : 16;
  var cardType = getCardTypeByValue(cardNumber);
  if (!cardType) return (cardNumber && cardNumber.match(/\d+/g) || []).join('').substr(0, maxLength);
  var format = cardType.format;
  if (format.global) {
    return cardNumber.match(format).join(delimiter).substr(0, maxLength);
  }
  var execResult = format.exec(cardNumber.split(delimiter).join(''));
  if (execResult) {
    return execResult.splice(1, 3).filter(function (x) {
      return x;
    }).join(delimiter).substr(0, maxLength);
  }
