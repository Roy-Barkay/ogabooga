...
define(['module', './is-api', 'require'], function (module, api, require) {
    ...
    //add 'browser' feature
    if (is.features.browser === undefined) is.features.browser = (typeof window != 'undefined');
    if (is.features.build === undefined) is.features.build = false;
    //build tracking
    is.curModule = null;
    is.modules = null;
    is.empty = function () {
    ...
    is.lookup = function (feature, complete) {
        ...
        require([feature], function (_feature) {
            if (_feature !== true && _feature !== false) throw 'Feature module ' + feature + ' must return true or false.';
    ...
    is.load = function (name, req, load, config) {
        ...
        if (f.type == 'lookup') is.lookup(f.feature, load);
        if (f.type == 'load_if' || f.type == 'load_if_not') {
            ...
            is.lookup(f.feature, function (_feature) {
                ...
                else if ((!_feature && f.type == 'load_if' && f.noModuleId) || (_feature && f.type == 'load_if_not' && f.noModuleId)) require([f.noModuleId], load);
                else load(is.empty());
            });
        }
    };
