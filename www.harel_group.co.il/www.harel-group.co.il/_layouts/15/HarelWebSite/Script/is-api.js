define(function () {
    ...
    api.normalize = function (name, normalize) {
        ...
        if (f.yesModuleId) f.yesModuleId = normalize(f.yesModuleId);
        if (f.noModuleId) f.noModuleId = normalize(f.noModuleId);
        //reconstruct
        if (f.type == 'lookup') return f.feature;
        if (!f.yesBuild) f.yesModuleId = '[' + f.yesModuleId + ']';
        if (!f.noBuild) f.noModuleId = '[' + f.noModuleId + ']';
        if (f.type == 'load_if_not') return '~' + f.feature + '?' + f.yesModuleId + (f.noModuleId ? ':' + f.noModuleId : '');
        if (f.type == 'load_if') return f.feature + '?' + f.yesModuleId + (f.noModuleId ? ':' + f.noModuleId : '');
    }
    /*
    ...
    api.parse = function (f) {
        ...
        if (actions.substr(yesModuleId.length + 1, 2) == '//') yesModuleId = actions.substr(0, actions.indexOf(':', yesModuleId.length + 1));
